...
import com.gs.demo.hr.GenericDao;
...
public abstract class GenericDaoImpl<T, ID extends Serializable> implements
	 ...
	 */
	@SuppressWarnings("unchecked")
	public T read(Class clazz, ID id) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public T readExclusive(Class clazz, ID id) {
	 ...
	 * {@inheritDoc}
	 */
	public T refreshById(final ID id) {
		if (getEntityBeanType() == null) {
			throw new UnsupportedOperationException(
					"The type must be set to use this method."); //$NON-NLS-1$
		}
