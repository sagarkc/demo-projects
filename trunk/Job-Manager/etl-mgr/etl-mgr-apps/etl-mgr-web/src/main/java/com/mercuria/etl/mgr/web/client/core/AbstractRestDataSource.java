...
public abstract class AbstractRestDataSource extends RestDataSource {
	 ...
	 * @param id
	 */
	public AbstractRestDataSource(String dataSourceId) {
	...
	protected void postProcessTransform(DSRequest request) {
		...
		//SC.say(URL.encode(url.toString() + WebConstants.DISPATCHER_EXT));
		request.setActionURL(URL.encode(url.toString() + WebConstants.RPC_EXT));
	}
