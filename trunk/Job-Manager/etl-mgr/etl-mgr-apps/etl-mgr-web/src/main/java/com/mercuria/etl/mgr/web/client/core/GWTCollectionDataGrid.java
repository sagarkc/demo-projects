...
public class GWTCollectionDataGrid<T extends GWTCollectionData> extends ListGrid{
	...
	private void initGrid() {
		...
		for (int i = 0; i < columnNames.size(); i++) {
			...
			ListGridField column = new ListGridField(
					model.getColumnFieldNameMap().get(colName), 
					colName, model.getColumnWidthMap().get(colName)
					);
			headers[i] = column;
	...
	private void setRowData() {
		...
		for(int r=0; r < model.getRowCount(); r++){
			...
			for(int c=0; c < model.getColumnCount(); c++){
				...
				record.setAttribute(model.getColumnFieldNameMap().get(colName), model.getValueAt(r, c));  
			}
			records[r] = record;
		}
		setData(records);
